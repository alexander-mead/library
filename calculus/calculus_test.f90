PROGRAM calculus_test
  ...
  REAL, PARAMETER :: acc=1d-5
  
  WRITE(*,*)

  !This program take a function (a -> b) and integrates it at equally
  !spaced intervals using the trapezium rule, it doubles the 
  !number of timesteps until it achieves desired accuracy
  ...
  ELSE IF(itest==2) THEN
     ...
     DO iorder=1,3,2
        ...
        sol=integrate_old(lim1,lim2,func,acc,iorder)
        WRITE(*,*) 'Order:', iorder
        WRITE(*,*) 'Standard:', sol
        WRITE(*,*) 'Accuracy:', sol/true
        WRITE(*,*)
     ...
     sol=cubeint(lim1,lim2,func,acc)
